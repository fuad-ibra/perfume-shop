(() => {
    ...
    function initSliders() {
        if (document.querySelector(".banner__slider")) new core(".banner__slider", {
            ...
            watchOverflow: true,
            navigation: {
                prevEl: ".slider-banner__arrow--prev",
                nextEl: ".slider-banner__arrow--next"
            },
            on: {
                slideChange: function(sld) {
                    document.body.setAttribute("data-sld", sld.realIndex);
                }
            }
        });
        if (document.querySelector(".bestsellers-slider")) new core(".bestsellers-slider", {
            ...
            navigation: {
                prevEl: ".bestsellers-arrow-prev",
                nextEl: ".bestsellers-arrow-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1
        ...
        if (document.querySelector(".novelties-slider")) new core(".novelties-slider", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (document.querySelector(".recommendations-slider")) new core(".recommendations-slider", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            navigation: {
                prevEl: ".recommendations-arrow-prev",
                nextEl: ".recommendations-arrow-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1
        ...
        if (document.querySelector(".sales-slider")) new core(".sales-slider", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            navigation: {
                prevEl: ".sales-arrow-prev",
                nextEl: ".sales-arrow-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1
        ...
        if (document.querySelector(".thumbs-images")) {
            ...
            new core(".images-product__slider", {
                modules: [ Navigation, Pagination, Thumb, Zoom ],
                autoplay: {
                    delay: 3e3,
    ...
    script_formQuantity();
    const favorites = document.querySelectorAll(".slider-products__favorite");
    favorites.forEach((elem => {
        elem.addEventListener("click", (function() {
            if (!elem.classList.contains("addFavorite")) elem.classList.add("addFavorite"); else elem.classList.remove("addFavorite");
        }));
    }));
    ...
    window.addEventListener("scroll", (function() {
        ...
        if (scrollTop > lastScrollTop) header.style.top = "-6.5rem"; else header.style.top = "0";
        lastScrollTop = scrollTop;
    }));
    const counters = document.querySelectorAll(".clients__count");
    const speed = 1e3;
    const countersParent = document.querySelector(".clients__col--right");
    const aboutWrapper = document.querySelector("#about-page");
    ...
    const dropdownCart = () => {
        const basketBtn = document.querySelector("[data-basket]");
        const basketProduct = document.querySelector("[data-product]");
        basketBtn.addEventListener("click", (() => {
            basketProduct.classList.toggle("product-active");
        }));
    };
    dropdownCart();
    const toggleCartEmptyText = () => {
        const textEmptyBadge = document.querySelector(".body-table__empty");
        const productCartWrapper = document.querySelector(".body-table__productWrapper");
        const productCartFooter = document.querySelector(".body-table__footer");
        if (productCartWrapper.children.length > 0) {
            textEmptyBadge.style.display = "none";
            productCartFooter.style.display = "block";
        } else {
            textEmptyBadge.style.display = "block";
            productCartFooter.style.display = "none";
        }
    };
    window.addEventListener("click", (function(e) {
        let numberEl = document.querySelector("[data-num]");
        if (e.target.classList.contains("body-table__icon")) {
            const parent = e.target.closest(".body-table__product");
            const quantity = parent.querySelector("[data-cart-quantity]");
            e.target.closest(".body-table__product").remove();
            numberEl.innerText = parseInt(numberEl.innerText) - parseInt(quantity.innerText);
        }
        toggleCartEmptyText();
    }));
    document.addEventListener("click", (function(e) {
        if (e.target.hasAttribute("data-icon-remove")) {
            console.log(e.target);
            e.target.closest("[data-product-wrapper]").remove();
        }
    }));
    document.querySelector("#cart-page");
    const getProductInfo = e => {
        if (e.target.hasAttribute("data-button") || e.target.hasAttribute("data-button-text")) {
            const card = e.target.closest(".main-product");
            const productInfo = {
                id: card.dataset.id,
                imgSrc: card.querySelector(".thumbs-images__slide--01 img").getAttribute("src"),
                title: card.querySelector(".header-product__title").innerText,
                count: card.querySelector(".quantity__input input").value,
                price: card.querySelector("#price").innerText
            };
            productInfo.id, productInfo.imgSrc, productInfo.title, productInfo.title, productInfo.count, 
            productInfo.price;
            const cartItem = `<div class="body-table__product body-table__product-cart" data-id="${productInfo.id}" data-product>\n\t\t\t\t\t\t\t\t\t<a class="body-table__image" href="product.html">\n\t\t\t\t\t\t\t\t\t\t<img src="${productInfo.imgSrc}" alt="${productInfo.title}">\n\t\t\t\t\t\t\t\t\t</a>\n\t\t\t\t\t\t\t\t\t<div class="body-table__content">\n\t\t\t\t\t\t\t\t\t\t<h3 class="body-table__title">\n\t\t\t\t\t\t\t\t\t\t\t${productInfo.title}\n\t\t\t\t\t\t\t\t\t\t</h3>\n\t\t\t\t\t\t\t\t\t\t<div class="body-table__info">\n\t\t\t\t\t\t\t\t\t\t\t<div class="body-table__item body-table__item--quantity">\n\t\t\t\t\t\t\t\t\t\t\t\t<span>Qty:</span> \n\t\t\t\t\t\t\t\t\t\t\t\t<span data-cart-quantity>${productInfo.count}</span>\t\n\t\t\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t\t\t<div class="body-table__item body-table__item--price">\n\t\t\t\t\t\t\t\t\t\t\t\t<span>&euro;</span>\n\t\t\t\t\t\t\t\t\t\t\t\t<span data-cart-price>${productInfo.price}</span>\n\t\t\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t\t\t<svg class="body-table__icon" width="16" height="18" viewBox="0 0 16 18">\n\t\t\t\t\t\t\t\t\t\t<path\n\t\t\t\t\t\t\t\t\t\t\td="M0.6875 3.9375H1.8125V15.4575C1.81398 15.9822 2.02307 16.4849 2.39407 16.8559C2.76507 17.2269 3.26783 17.436 3.7925 17.4375H12.23C12.7508 17.4301 13.2478 17.2184 13.6139 16.8481C13.9801 16.4777 14.1861 15.9783 14.1875 15.4575V3.9375H15.3125C15.4617 3.9375 15.6048 3.87824 15.7102 3.77275C15.8157 3.66726 15.875 3.52418 15.875 3.375C15.875 3.22582 15.8157 3.08274 15.7102 2.97725C15.6048 2.87176 15.4617 2.8125 15.3125 2.8125H11.375V1.6875C11.3751 1.39394 11.2604 1.11197 11.0555 0.901786C10.8506 0.691598 10.5716 0.569839 10.2781 0.5625H5.75C5.45163 0.5625 5.16548 0.681026 4.9545 0.892005C4.74353 1.10298 4.625 1.38913 4.625 1.6875V2.8125H0.6875C0.538316 2.8125 0.395242 2.87176 0.289752 2.97725C0.184263 3.08274 0.125 3.22582 0.125 3.375C0.125 3.52418 0.184263 3.66726 0.289752 3.77275C0.395242 3.87824 0.538316 3.9375 0.6875 3.9375ZM5.75 1.6875H10.25V2.8125H5.75V1.6875ZM4.625 3.9375H13.0625V15.4575C13.0625 15.6843 12.9724 15.9017 12.8121 16.0621C12.6517 16.2224 12.4343 16.3125 12.2075 16.3125H3.77C3.54716 16.3066 3.33543 16.214 3.17992 16.0543C3.02441 15.8945 2.93742 15.6804 2.9375 15.4575V3.9375H4.625Z"\n\t\t\t\t\t\t\t\t\t\t\tfill="#a1a1a1">\n\t\t\t\t\t\t\t\t\t\t</path>\n\t\t\t\t\t\t\t\t\t\t<path\n\t\t\t\t\t\t\t\t\t\t\td="M6.13281 14.0625C6.282 14.0625 6.42507 14.0032 6.53056 13.8977C6.63605 13.7923 6.69531 13.6492 6.69531 13.5V6.75C6.69531 6.60082 6.63605 6.45774 6.53056 6.35225C6.42507 6.24676 6.282 6.1875 6.13281 6.1875C5.98363 6.1875 5.84055 6.24676 5.73506 6.35225C5.62958 6.45774 5.57031 6.60082 5.57031 6.75V13.5C5.57031 13.6492 5.62958 13.7923 5.73506 13.8977C5.84055 14.0032 5.98363 14.0625 6.13281 14.0625Z"\n\t\t\t\t\t\t\t\t\t\t\tfill="#a1a1a1">\n\t\t\t\t\t\t\t\t\t\t</path>\n\t\t\t\t\t\t\t\t\t\t<path\n\t\t\t\t\t\t\t\t\t\t\td="M9.86719 14.0625C10.0164 14.0625 10.1594 14.0032 10.2649 13.8977C10.3704 13.7923 10.4297 13.6492 10.4297 13.5V6.75C10.4297 6.60082 10.3704 6.45774 10.2649 6.35225C10.1594 6.24676 10.0164 6.1875 9.86719 6.1875C9.718 6.1875 9.57493 6.24676 9.46944 6.35225C9.36395 6.45774 9.30469 6.60082 9.30469 6.75V13.5C9.30469 13.6492 9.36395 13.7923 9.46944 13.8977C9.57493 14.0032 9.718 14.0625 9.86719 14.0625Z"\n\t\t\t\t\t\t\t\t\t\t\tfill="#a1a1a1">\n\t\t\t\t\t\t\t\t\t\t</path>\n\t\t\t\t\t\t\t\t\t</svg>\n\t\t\t\t\t\t\t\t</div>`;
            addProductToHeaderCart(cartItem, productInfo);
            toggleCartEmptyText();
            calcCartPrice();
            card.querySelector(".quantity__input input").value = "1";
        }
    };
    const calcCartPrice = () => {
        const cartItems = document.querySelectorAll(".body-table__product-cart");
        const totalPrice = document.querySelector("[data-total");
        let total = 0;
        cartItems.forEach((item => {
            const countEl = item.querySelector("[data-cart-quantity]");
            const priceEl = item.querySelector("[data-cart-price]");
            const currentPrice = parseInt(countEl.innerText) * parseFloat(priceEl.innerText);
            total += currentPrice;
        }));
        totalPrice.innerText = total.toFixed(2);
    };
    const addProductToHeaderCart = (cartItem, productInfo) => {
        const productCartWrapper = document.querySelector(".body-table__productWrapper");
        const itemInCart = productCartWrapper.querySelector(`[data-id="${productInfo.id}"]`);
        if (itemInCart) {
            const counterElement = itemInCart.querySelector("[data-cart-quantity]");
            counterElement.innerText = parseInt(counterElement.innerText) + parseInt(productInfo.count);
        } else productCartWrapper.insertAdjacentHTML("afterbegin", cartItem);
        addProductCountInCart();
    };
    const addProductCountInCart = () => {
        const numberEl = document.querySelector("[data-num]");
        const counterElement = document.querySelector("[data-cart-quantity]");
        numberEl.innerText = parseInt(counterElement.innerText);
    };
    window.addEventListener("click", getProductInfo);
    window["FLS"] = true;
    isWebp();
    addTouchClass();
    addLoadedClass();
